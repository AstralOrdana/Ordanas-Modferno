...
public class ModBasaltPillar extends PointedDripstoneBlock implements LandingBlock, Waterloggable {
    ...
    public ModBasaltPillar(Settings settings) {
        super(settings);
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (random.nextFloat() < 0.011377778F && isHeldByBasaltPillar(state, world, pos)) {
    ...
    public static void dripTick(BlockState state, ServerWorld world, BlockPos pos, float dripChance) {
        if (!(dripChance > 0.17578125F) || !(dripChance > 0.05859375F)) {
            if (isHeldByBasaltPillar(state, world, pos)) {
    ...
    private static void place(WorldAccess world, BlockPos pos, Direction direction, Thickness thickness) {
        BlockState blockState = (BlockState)((BlockState)((BlockState)ModBlocks.BASALT_PILLAR.getDefaultState().with(VERTICAL_DIRECTION, direction)).with(THICKNESS, thickness)).with(WATERLOGGED, world.getFluidState(pos).getFluid() == Fluids.WATER);
    ...
    private static BlockPos getTipPos(BlockState state, WorldAccess world, BlockPos pos, int range, boolean allowMerged) {
        if (isTip(state, allowMerged)) {
            ...
            Predicate<BlockState> predicate = (statex) -> {
                return statex.isOf(ModBlocks.BASALT_PILLAR) && statex.get(VERTICAL_DIRECTION) == direction;
            };
    ...
    private static Thickness getThickness(WorldView world, BlockPos pos, Direction direction, boolean tryMerge) {
        ...
        if (isBasaltPillarFacingDirection(blockState, direction2)) {
            return !tryMerge && blockState.get(THICKNESS) != Thickness.TIP_MERGE ? Thickness.TIP : Thickness.TIP_MERGE;
        } else if (!isBasaltPillarFacingDirection(blockState, direction)) {
            ...
            if (thickness != Thickness.TIP && thickness != Thickness.TIP_MERGE) {
                ...
                return !isBasaltPillarFacingDirection(blockState2, direction) ? Thickness.BASE : Thickness.MIDDLE;
            } else {
    ...
    private static Optional<BlockPos> getSupportingPos(World world, BlockPos pos, BlockState state, int range) {
        ...
        return searchInDirection(world, pos, direction.getOpposite().getDirection(), predicate, (statex) -> {
            return !statex.isOf(ModBlocks.BASALT_PILLAR);
        }, range);
    }
    ...
    private static boolean canPlaceAtWithDirection(WorldView world, BlockPos pos, Direction direction) {
        ...
        BlockState blockState = world.getBlockState(blockPos);
        return blockState.isSideSolidFullSquare(world, blockPos, direction) || isBasaltPillarFacingDirection(blockState, direction);
    }

    private static boolean isTip(BlockState state, boolean allowMerged) {
        if (!state.isOf(ModBlocks.BASALT_PILLAR)) {
            return false;
        } else {
            Thickness thickness = (Thickness)state.get(THICKNESS);
    ...
    private static boolean isPointingDown(BlockState state) {
        return isBasaltPillarFacingDirection(state, Direction.DOWN);
    }

    private static boolean isPointingUp(BlockState state) {
        return isBasaltPillarFacingDirection(state, Direction.UP);
    }

    private static boolean isHeldByBasaltPillar(BlockState state, WorldView world, BlockPos pos) {
        return isPointingDown(state) && !world.getBlockState(pos.up()).isOf(ModBlocks.BASALT_PILLAR);
    }
    ...
    private static boolean isBasaltPillarFacingDirection(BlockState state, Direction direction) {
        return state.isOf(ModBlocks.BASALT_PILLAR) && state.get(VERTICAL_DIRECTION) == direction;
    }
    ...
    private static BlockPos getCauldronPos(World world, BlockPos pos, Fluid fluid) {
        ...
        return (BlockPos)searchInDirection(world, pos, Direction.DOWN.getDirection(), AbstractBlockState::isAir, predicate, 11).orElse(null);
    }

    @Nullable
    public static BlockPos getDripPos(World world, BlockPos pos) {
        return (BlockPos)searchInDirection(world, pos, Direction.UP.getDirection(), AbstractBlockState::isAir, ModBasaltPillar::canDrip, 11).orElse(null);
    }

    public static Fluid getDripFluid(World world, BlockPos pos) {
        return (Fluid)getFluid(world, pos, world.getBlockState(pos)).filter(ModBasaltPillar::isFluidLiquid).orElse(Fluids.EMPTY);
    }
    ...
    private static boolean canGrow(BlockState basaltBlockState, BlockState lavaState) {
        return basaltBlockState.isOf(Blocks.BASALT) && lavaState.isOf(Blocks.LAVA) && lavaState.getFluidState().isStill();
    }
