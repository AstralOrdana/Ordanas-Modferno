...
public class NetherCrops extends PlantBlock implements Fertilizable {
    ...
    protected NetherCrops(Settings settings) {
        super(settings);
    ...
    protected boolean canPlantOnTop(BlockState floor, BlockView world, BlockPos pos) {
        return floor.isOf(ModBlocks.NETHER_FARMLAND);
    }
    ...
    protected static float getAvailableMoisture(Block block, BlockView world, BlockPos pos) {
        ...
        for(int i = -1; i <= 1; ++i) {
            for(int j = -1; j <= 1; ++j) {
                ...
                if (blockState.isOf(ModBlocks.NETHER_FARMLAND)) {
                    g = 1.0F;
                    if ((Integer)blockState.get(NetherFarmland.FERTILITY) > 0) {
                        g = 3.0F;
                    }
                }
